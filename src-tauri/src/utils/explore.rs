...
pub async fn explore_movies_folder(stream: &mut FtpStream, media_type: MediaType, folder: Option<&str>) -> Vec<Movie> {
    ...
    let existing_movies = load_movies(stream, match media_type {
...
pub async fn explore_series_folder(stream: &mut FtpStream, folder: Option<&str>) -> Vec<Serie> {
    ...
    while let Some(path) = stack.pop_back() {
        ...
        for file in nlst {
            ...
            let serie = match search_serie(&title, date).await {
                Ok(serie) => Serie {
                    path: Some(format!("{}/{}", path, file)),
                    ..serie
                },
            ...
            let seasons = explore_seasons_folder(stream, &serie, None).await;
            series.push(Serie {
...
pub async fn explore_seasons_folder(stream: &mut FtpStream, serie: &Serie, folder: Option<&str>) -> Vec<Season> {
    let initial_path = folder
    ...
    while let Some(path) = stack.pop_back() {
        ...
        for file in nlst {
            ...
            let season = match search_season(serie.id, season_number.parse().unwrap()).await {
                Ok(season) => Season {
                    path: Some(format!("{}/{}", path, file)),
                    ..season
                },
            ...
            let episodes = explore_episodes(stream, serie, &season, None).await;
            seasons.push(Season {
...
pub async fn explore_episodes(
    ...
    let existing_series = load_series(stream);
    ...
    while let Some(path) = stack.pop_back() {
        ...
        for file in nlst {
            ...
            match search_episode(serie.id, season.season_number, episode_number.parse().unwrap()).await {
